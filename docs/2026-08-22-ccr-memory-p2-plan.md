...
- v14 迁移：幂等（safeAddColumn 先 pragma 检查）；`cached_tokens` 写入端 = llm-controller 落库路径 + 三处渲染层 log-llm-call（ccr-summary/chapter-memory/agent-store）；node:sqlite 内存 DB 验证（项目测试环境 better-sqlite3 与 vitest Node ABI 不兼容——P2 前已验证可行）
...
```ts
// electron/vector-store.test.ts（或既有测试文件追加）
import { describe, it, expect } from 'vitest'
import { parseChapterFromFileNameForBackfill } from './vector-store' // 导出回填解析（复用 knowledge-base parseChapterMetaFromFileName 语义）

describe('存量回填章节号解析', () => {
  it('第N章 标题.txt → 章节号', () => {
    expect(parseChapterFromFileNameForBackfill('第 9 章 破坛换晶.txt')).toBe(9)
    expect(parseChapterFromFileNameForBackfill('第1章开局.txt')).toBe(1)
  })

  it('无匹配 → null（回填 NULL，scopeFilter 容忍）', () => {
    expect(parseChapterFromFileNameForBackfill('设定集.md')).toBeNull()
    expect(parseChapterFromFileNameForBackfill('chapter_9.txt')).toBeNull()
  })
})
```
...
import { parseChapterMetaFromFileName } from './knowledge-base' // 若跨文件依赖有循环风险则复制纯函数（含测试）

/** 存量表回填章节号解析（fileName → chapterNumber；无匹配 null） */
export function parseChapterFromFileNameForBackfill(fileName: string): number | null {
  const meta = parseChapterMetaFromFileName(fileName)
...
export async function ensureChunksSchema(db: LanceDB): Promise<{ migrated: boolean; error?: string }> {
  try {
    ...
    const updates = rows.map((r: { id: string; fileName?: string }) => ({
      id: r.id,
      chapterNumber: r.fileName ? parseChapterFromFileNameForBackfill(r.fileName) : null,
    }))
...
**Files:**
- Modify: `electron/database.ts`（v14 迁移段 + CURRENT_SCHEMA_VERSION → 14）
...
describe('v14 cached_tokens 迁移', () => {
  it('加列幂等（重复执行不报错）', () => {
    ...
    // 迁移段（从 database.ts 提取的 v14 SQL，含 safeAddColumn 语义）
...
- [ ] **Step 2: database.ts 迁移段**（v13 → v14：safeAddColumn 'llm_calls' 'cached_tokens' + CURRENT_SCHEMA_VERSION 14——照 db-migration-standard 幂等/哨兵 checklist）

- [ ] **Step 3: 三处写入端补字段**（ccr-summary.ts / chapter-memory.ts 成功落库 + agent-store.ts 成功/失败落库——`cached_tokens: usage?.cachedTokens ?? 0`；主进程 llm-controller 落库路径若已透传则无需改）
...
- Produces:
  ...
  - 触发：每 3-5 分卷检查点（卷号 % 3 === 0 时在卷聚合后触发——低频）；查看器手动重建按钮（P1 仅标 stale → 改为真实重建）
...
**Interfaces:**
- Produces: `ensureVolumeSummary` 对进行中卷（chapterEnd === 0）：聚合区间 = chapterStart..当前最大章节号（memory:list 各 chapters 文件 range 解析的最大 end）；沿用 stale 过滤 + 完整性检查（区间内条目齐全才写）

- [ ] **Step 1: 写失败测试**（进行中卷 16 起、当前最大章 25 → 聚合 16-25 条目 → volume-002.md 生成）

- [ ] **Step 2: 实现**（进行中卷分支：最大章节号 = memory:list 中 chapters 文件 range 最大 end；完整性检查同 P1）
...
| llm_calls.cached_tokens 列——CacheAligner 效果可事后统计 | Task 2 |
| book-state 全书摘要自动生成（每 3-5 分卷/手动） | Task 3 |
| 进行中卷卷级聚合（解除 P1 限制） | Task 4 |
| 记忆手动编辑（查看器编辑模式） | Task 5 |
| 全局 token 统计面板（purpose/模型/时间） | Task 6 |

**范围外（P3 候选）**：跨会话记忆复用（SharedContext 式——最大项，独立一期）、记忆查看器 AI 面板入口（设计 §5.4 双入口的第二入口）。
