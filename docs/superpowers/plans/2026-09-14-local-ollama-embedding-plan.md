...
- [x] **Step 4: 文档回更（T3 review M1/M7 债）** —— 已全部落在**设计文档 §9.2 的回更表**（逐条 before/after）：
  ...
  - **W3 附带订正**：设计 §9.4 原写 `addChunks` 的维度守卫「在真实路径上永远跑不到」**过于绝对**——整支复审用真实探针证否；W3 实测后改写为「**仅在库由章节化首导诞生时可达**」，并订正剪枝机制（剪枝发生在 **schema 推断**期：`undefined` 键不产生列；显式 schema 下的全 `null` 列其实**会被保留**）。  - 本计划文件自身：把「检索侧适配」从 v2 移出（T3b 已做）。
