...
- [x] **Step 4: 文档回更（T3 review M1/M7 债）** —— 已全部落在**设计文档 §9.2 的回更表**（逐条 before/after）：
  ...
  - **代码注释同步**：`electron/vector-store.ts:1178` 的注释同样写着「1024 写 1536 列 = 客户端 `TypeError`」的旧表述，与实测不符。**T6 的硬边界是不改生产代码**（改代码会让本次门禁与真机结论脱离最终产物），故 T6 当时**只在设计文档 §9.5 登记为「已知与实测冲突的代码注释（待随下次代码改动订正）」**，代码注释保持原样。→ **✅ 已由 final wave（W2）落地订正**：该 commit 把注释改写为两条写入路径的真实失败模式（`table.update` 静默写 `null`；`table.add` 静默截断/补零且**不改列维度**），并点明这正是写前守卫存在的理由；设计 §9.5 同步改为「已订正」。**T6 自身仍未改任何生产代码**（本计划的 T6 边界未被违反，订正发生在 T6 之后的修复波）。
  - **W3 附带订正（经 R2 二次订正）**：设计 §9.4 原写 `addChunks` 的维度守卫「在真实路径上永远跑不到」**过于绝对**——整支复审用真实探针证否。W3 把它改写为「仅在库由章节化首导诞生时可达」，但 W3 给的**机制**（「剪枝发生在 schema 推断期」）本身是错的；**R2 已按 2×3 矩阵探针订正**为：**某列被剪当且仅当每条记录该字段全为 `undefined`，与是否显式 schema 无关**（`null` 是值：显式 schema 下保留、推断时抛错）；`addChunks` **首次建表也传显式 schema**，全 `undefined` 列在那条路上照样被剪。同时补上形态 2 的**第二个必需条件（首导必须带向量**——`requiredFields` 含 `vector`），并把「可达性」正名为**每次导入判定、可随后续重建翻转**。详见设计 §9.4。
  - 另注：上一条（`:269`）沿用 T6 口径把该守卫称作「**死代码**」——R2 后更准确的说法是「**仅在特定数据形状下可达**」，见设计 §9.4。
