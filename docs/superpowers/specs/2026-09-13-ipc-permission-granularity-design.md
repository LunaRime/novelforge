...
| **S8b（待接线）** | 把 `grants` 接进 `fs-controller`：10 处 `validateSandbox` 收敛为 `assertPathAllowed`；`electron/utils/active-project.ts` 作为项目根单一真源；4 处对话框处理器内部登记授权；**删除** `fs:grant-external-file` 及其 2 个调用点（`FilePickerMenu.tsx:49`、`audit-context.ts:80`）；保留 `fs-controller.ts:463` 的 agent-result 自登记 | ⚠️ 行为变更 | 回归清单全过（对话框授权/外部文件/导出/分享卡/工作流输出/主目录外项目/记忆读写删） |
