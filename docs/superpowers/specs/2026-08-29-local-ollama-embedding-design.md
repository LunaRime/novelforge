...
- **建议修法（未实施，待裁决）**：① 下拉改为「可输入 + 建议列表」（允许自由输入模型名）；② 或让下拉在未安装时也列出一小组已知 embedding 模型（`bge-m3` / `nomic-embed-text` / `mxbai-embed-large` 等）供一键下载。② 更贴合「程序自动下载」的预期。
...
10. ⚠️ **前置：下拉是封闭选择**——选项 = 已安装模型 ∪ 配置里的模型名（`VectorConfigSection.tsx` 的 `modelOptions`），**界面无法自由输入模型名**。要构造「不存在的模型」：把 `%USERPROFILE%\.novelforge\config.json` 的 `localEmbedding.model` 改成 `definitely-not-a-model`，**关掉设置页再重新打开**（卡片挂载时才重读配置）→ 徽标变「已连接，但模型 … 未安装」→ 再点「下载模型」。
