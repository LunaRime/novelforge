...
- **真机复测（2026-09-14，用户侧）**：修复后设置页「测试模型」**成功**（bge-m3 / 1024 维）。CI run `35091714876`（headSha `06a3316`）三平台全绿。→ 本缺陷**闭环**。C–J 组仍待续。

### 9.7 真机发现：通道 A（应用内下载）实际只能下载「配置里的那个模型名」

- **现象**（2026-09-14 真机 D 组）：模型下拉里**只有 `bge-m3`**，既选不了也输不进其它名字，因此无法按原步骤构造「拉取一个不存在的模型」。
- **机制**：`VectorConfigSection.tsx` 的 `modelOptions` = **已安装模型 ∪ 配置里的 `model`**（封闭选择），而「下载模型」按钮下载的是**当前选中的模型**。
- **推论**：对尚未安装的模型，只有当它就是「配置里的 `model`」时才能从应用内下载。用户想装 `nomic-embed-text` 之类的其它模型，**只能回到终端 `ollama pull`** —— 即 UI 文案宣称的「双通道」在默认模型之外的情形下退化为单通道，与「零命令行」的体验预期不符。
- **性质**：不是数据/正确性问题，是**可达性**问题，也是用户「太麻烦了」抱怨的具体来源之一。
- **✅ 已修（`381f0b8` + Minor 收口轮）**：下拉改为**内置精选清单**（`bge-m3` / `nomic-embed-text` / `mxbai-embed-large` / `all-minilm`，各带参数规模、一句话定位、已安装/可下载徽标），**每项都有自己的下载入口** → 通道 A 不再只服务配置里的那一个模型。
  - **自由输入被明确否决**（用户裁决：「用户不知道有哪些向量模型可以下载，也不知道哪些模型好」）→ 清单是**封闭**的；清单外的历史配置仍作为额外一项渲染并保持选中（不把用户锁在清单外）。
  - **实现约束与取舍**：`embedding:local-pull` 的 invoke args 是硬性 `[]`（T4 契约，渲染层无法传模型名）→「下载某一项」只能实现为 **先 `local-set-config { model: 该项 }` 再 `local-pull`**，即**点下载会同时把该项切为当前模型**。经独立 review 裁定**可接受**：写配置是同步原子写（无竞态）；下载失败时卡片显示「模型未安装」且该行可重试；导入侧照常降级 api→llm→fts，不报错不半写入；**回滚反而更糟**（会静默撤销用户的显式选择）。为消除唯一的诚实性缺口，按钮文案改为**「下载并切换为 {model}」**（Minor 4），参数规模标注为「{n} 参数」以免被读成下载体积（Minor 3）；地址为默认值时不再显示无效的「重置为默认」（Minor 5）。
  - **未采用的方案**：① 自由输入（已被用户否决）；② 改 `local-pull` 的 payload 加模型名——需同时改 L4 三处（声明 / 策略表 / 类型侧），成本明显更高，本轮不做。

## 10. 真机验证清单（人工执行；**2026-09-14 起部分执行：A 组通过，B 组暴露的冷启动缺陷已修并在真机复测通过（「测试模型」成功），C–J 待续**）
...
10. 构造「下载失败」：在清单里选一个**未安装**的项（如 `nomic-embed-text`），先**断开网络**（或停掉 Ollama），再点「下载并切换为 …」。
    - ⚠️ **不要再改 `config.json`** 构造不存在的模型名 —— R1 之后清单里任何未安装项都有自己的下载入口（§9.7 已修）。改用手改配置的那个步骤已删除。
