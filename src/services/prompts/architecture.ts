...
【输出纪律（违反任一即视为不合格）】
- 严格按 chapterNumber 从 1 到 {{number_of_chapters}} 逐章输出，一行一章；严禁遗漏、跳号或重复任何一章。
- 严禁输出 1-{{number_of_chapters}} 之外的章节。
- keyEvents 中的角色、设定、金手指必须直接来自【全书架构数据池】，严禁凭空引入架构中不存在的角色、势力或新设定。
- 控制整批输出总长度：每章约 1 行（keyEvents 100-150 字），整批约 {{number_of_chapters}} 行，不要无谓扩写。
...
【输出纪律（违反任一即视为不合格）】
- 严格只输出 chapterNumber 为 {{n}} 到 {{m}} 的每一章，一行一章；严禁输出 {{m}} 之后的章节（不得提前泄露后续情节），严禁遗漏、跳号或重复。
- keyEvents 中的角色、设定、金手指必须直接来自【全书架构数据池】或【前置剧情进度】，严禁凭空引入架构中不存在的角色、势力或新设定。
- 控制整批输出总长度：每章约 1 行（keyEvents 100-150 字），整批约 {{m}}−{{n}}+1 行，不要无谓扩写。
