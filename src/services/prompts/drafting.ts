...
【具体生成要求】
- 体量与节奏控制：大约 {{word_number}} 字左右（字数由系统自动统计核对，你无需逐字计数，凭体感把握篇幅即可）。本章仅推演【本章信息】中规定的核心剧情，切忌注水！不要为了凑字数而撰写冗余的旁白科普或无意义的日常对话。达成首章目标后立刻留悬念断章，绝不可提前泄露后续情节。
...
3. 落实本章核心冲突：用{{word_number}}字左右的篇幅（字数由系统自动统计核对，你无需逐字计数，凭体感把握），踏踏实实地推演完本章目标，拒绝平淡流水账。
...
【输出格式】
- 体量与节奏控制：大约 {{word_number}} 字左右（字数由系统自动统计核对，你无需逐字计数，凭体感把握篇幅即可）。本章仅推演【本章信息】中规定的核心冲突，切忌注水！达成目标即立刻断章，绝不可擅自拓展后续大纲的情节。
