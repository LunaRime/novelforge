...
describe('IPC 通道对账（L4 §6 第 1 类）', () => {
  ...
  it('④ 通道数量快照（防无声增删；新增通道时同步更新）', () => {
    expect(registered.size).toBe(226) // T4：+6 embedding:local-*；2026-09-25：+4 供应商账户、+1 标题栏覆盖层；D 档：+13 db:automation-*；B2：+3 fs:agent-archive-original-*
    expect(declared.size).toBe(234) // 226 invoke + 8 event（T4：+embedding:local-pull-progress）
  })
  ...
  it('⑤ 策略表与已注册集合双向一致', () => {
    ...
    expect(policyKeys.size).toBe(226) // 2026-09-25：+4 供应商账户通道、+1 标题栏覆盖层；D 档：+13 db:automation-*；B2：+3 fs:agent-archive-original-*
  })
