...
describe('IPC 通道对账（L4 §6 第 1 类）', () => {
  ...
  it('④ 通道数量快照（防无声增删；新增通道时同步更新）', () => {
    expect(registered.size).toBe(205) // T4：+6 embedding:local-*
    expect(declared.size).toBe(213) // 205 invoke + 8 event（T4：+embedding:local-pull-progress）
  })
  ...
  it('⑤ 策略表与已注册集合双向一致', () => {
    ...
    expect(policyKeys.size).toBe(205)
  })
